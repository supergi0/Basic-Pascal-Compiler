...
logical_or_expression:
    ...
    logical_or_expression T_OR logical_and_expression
    {
        ...
        if($1.integer_val==1 || $3.integer_val==1){
            $$.integer_val = 1;
        }
        else{
            $$.integer_val = 0;
...
relational_expression:
    ...
    relational_expression LESS additive_expression
    {
        ...
        if($1.integer_val < $3.integer_val){
            $$.integer_val = 1;
        }
        else{
            $$.integer_val = 0;
...
identifier_or_array:
    identifier
    {
        ...
        for (int i = 0; i < count; i++) {
            if (strcmp(symbol_table[i].name, $1.nd->children[0]->token) == 0) {
                if($$.type == 1){
                    $$.integer_val = atoi(symbol_table[i].value);
                }
                if($$.type == 2){
                    $$.real_val = atof(symbol_table[i].value);
                }
                if($$.type == 3){
                    $$.character_val = symbol_table[i].value[0];
                }
            }
        }
    ...
    identifier LBRACKET expression RBRACKET 
    {
        ...
        for (int i = 0; i < count; i++) {
            char buffer[256];
            sprintf(buffer,"%s[%d]",$1.nd->children[0]->token,$3.integer_val);
...
for_statement:
    ...
    compound_statement
    {
        ...
        if($4.integer_val <= $6.integer_val){
...
write_params:
    T_STRING
    {
        ...
        char *token_copy = strdup($1.nd->children[0]->token);
        if (token_copy[0] == '\"') {
    ...
    write_params COMMA T_STRING
    {
        ...
        char *token_copy = strdup($3.nd->children[0]->token);
        if (token_copy[0] == '\"') {
            memmove(token_copy, token_copy + 1, strlen(token_copy));
        }
        if (token_copy[strlen(token_copy) - 1] == '\"') {
...
read_statement:
    READ LPAREN identifier_or_array RPAREN
    {
        ...
        for (int i = 0; i < count; i++) {
            char buffer[256];
            if($3.is_array==0){
                strcpy(buffer,$3.nd->children[0]->children[0]->token);
            }
            else{
                sprintf(buffer,"%s[%d]",$3.nd->children[0]->children[0]->token,$3.array_index);
            }
